...
class TreeNodeSolution {
    ...
    */
    ...
    func sumNumbers(_ root: TreeNode?) -> Int {
        sumDip(root, previousSum: 0)
    }
    
    func sumDip(_ root: TreeNode?, previousSum: Int) -> Int {
        guard let root = root else {
            return 0
        }
        let sum = previousSum * 10 + root.val
        if root.left == nil, root.right == nil {
            return sum
        } else {
            return sumDip(root.left, previousSum: sum) + sumDip(root.right, previousSum: sum)
        }
    }
    ...
    func averageOfLevels(_ root: TreeNode?) -> [Double] {
        ...
        while !queue.isEmpty {
            ...
            results.append(sum / Double(size))
        }
        return results
    }
